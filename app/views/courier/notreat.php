    ...
    <div class="container">
        <div class="row my-4">
            <div class="alert alert-danger text-center col-10 p-2 mx-auto" style='font-size: 0.8em;'>
                <?= $nbrCourierNoTraite ?> couriers sont en retard d'exécution
            </div>
        </div>
        ...
        <div class="row">
            <div class="table-responsive">
                <table class="table" id="orderTable" style="font-size: 14px;">
                    ...
                    <tbody>
                        <?php foreach($allCouriersTempsEcoule as $courier): ?>
                            <tr class="<?= ($courier->priority === ARRAY_PRIORITY[1]) ? 'fw-bold text-info':'' ?>" data-id="<?= $courier->courier_id ?>" style="cursor: pointer;">
                                ...
                                <td class="click" data-id="<?= $courier->courier_id ?>" <?= ($courier->priority === ARRAY_PRIORITY[1]) ? STYLE_TDS : '' ?> >
                                    <?= ($courier->reception_num !== '') ? $courier->reception_num : '-' ?>
                                </td>
                                <td class="click" data-id="<?= $courier->courier_id ?>" <?= ($courier->priority === ARRAY_PRIORITY[1]) ? STYLE_TDS : '' ?> >
                                    <?= $courier->category ?>
                                </td>
                                ...
                                <td class="click" data-id="<?= $courier->courier_id ?>" <?= ($courier->priority === ARRAY_PRIORITY[1]) ? STYLE_TDS : '' ?>  class="" >
                                    <?= $courier->status ?>
                                </td>
                                <td class="click" data-id="<?= $courier->courier_id ?>" <?= ($courier->priority === ARRAY_PRIORITY[1]) ? STYLE_TDS : '' ?> >
                                    <?= $courier->saved_by ?>
