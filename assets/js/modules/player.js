document.addEventListener("DOMContentLoaded", () => {
  ...
  let currentIndex = 0;
  playBtn.innerHTML =
    '<i id="play-icon" class="fa-solid fa-play text-xl ml-1"></i>';
  ...
  function updateVolumeIcon(val) {
    volumeIcon.className =
      "fa-solid text-gray-500 text-xs transition-colors group-hover:text-primary ";
    if (val == 0) {
  ...
  const getTrackData = (element) => ({
    ...
    ssd: element.getAttribute("data-ssd"),
  ...
  const playTrack = (element) => {
    ...
    audio.addEventListener("timeupdate", () => {
      ...
      if (progress >= 90) {
        if (data.es) {
          fetch(
            `/enseignement/add_view/${data.es}?sr=${data.sr}&ssd=${data.ssd}`,
          )
            .then((response) => {
              if (!response.ok)
                console.warn("Erreur lors de l'ajout de la vue");
            })
            .catch((error) =>
              console.error("Erreur réseau pour les statistiques:", error),
            );
        }
      }
    });
    ...
    audio.play();
    playBtn.innerHTML =
      '<i id="play-icon" class="fa-solid fa-pause text-xl"></i>';
